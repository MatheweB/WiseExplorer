...
class MoveStatistics:
    ...
    def score(self) -> float:
        ...
        mean = (self.wins + 0.5 * self.ties - self.losses) / n

        # Maximum-entropy variance for bounded utility in [-1, 1]
        variance = (1 - mean * mean) / (n + 1)

        s = mean - math.sqrt(variance)
        return (s + 1)/2  # (normalized to [0,1])
...
class GameMemory:
    ...
    def __init__(
        ...
        logger.info(f"GameMemory ({mode}): {db_path}")
    ...
    def _write_transition(
        ...
        try:
            ...
            Position.get_or_create(
                ...
                defaults={"board_bytes": before_state.board.tobytes(), "meta_json": "{}"},
            )
            Position.get_or_create(
                game_id=game_id,
                position_hash=canon_after.hash,
                defaults={"board_bytes": after_state.board.tobytes(), "meta_json": "{}"},
            )
    ...
    def get_best_move_with_score(self, game: GameBase, debug: bool = False) -> Optional[Tuple[Any, float]]:
        result = self._select_move(game, best=True, debug=debug)
        return result

    def get_worst_move_with_score(self, game: GameBase, debug: bool = False) -> Optional[Tuple[Any, float]]:
        result = self._select_move(game, best=False, debug=debug)
        return result

    def _select_move(self, game: GameBase, best: bool, debug: bool) -> Optional[Tuple[Any, float]]:
        game_id = game.game_id()
        ...
        for move in game.valid_moves():
            ...
            evaluations.append(TransitionEvaluation(
                move=move,
    ...
    def _get_transitions_cached(self, game_id: str, from_hash: str) -> Dict[str, PlayStats]:
        ...
        query = (
            ...
            .join(PlayStats, on=(TransitionRow.transition_hash == PlayStats.transition_hash))
            .where(
    ...
    def _get_parent_cached(self, game_id: str, position_hash: str, player_to_move: int) -> Optional[ParentNode]:
    ...
    def _render_debug(self, state: GameState, evaluations: List[TransitionEvaluation], chosen_hash: str) -> None:
        debug_rows = []
