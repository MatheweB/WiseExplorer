...
class GameMemory:
    ...
    def _best_opponent_reply(
        ...
        board: np.ndarray, 
        opponents: List[int]
    ) -> Tuple[Optional[float], Optional[float], Optional[float], Optional[float], Optional[float], Optional[float], Optional[List[int]]]:
        """
    ...
    def get_best_move(self, game_id: str, game_state: GameState, debug_move=False):
        ...
        players_query = PlayStats.select(PlayStats.snapshot_player).where(
            PlayStats.game_id == gid
        ).distinct()
        players_in_game = [int(r.snapshot_player) for r in players_query]
        ...
        for row in rows:
            ...
            if next_board is not None:
                ...
                opps_query = PlayStats.select(PlayStats.snapshot_player).where(
                    (PlayStats.game_id == gid) 
                    & (PlayStats.board_hash == next_hash)
                ).distinct()
                opps_with_data = [
                ...
                if opps_with_data:
                    opp_pW, opp_pT, opp_pN, opp_pL, opp_util_for_them, opp_cert, opp_move = self._best_opponent_reply(
                        gid, next_board_norm, opps_with_data
                    )
            ...
            dangerous = False
            if opp_util is not None and opp_cert is not None:
                if -opp_util >= 0.9 and opp_cert >= 0.8:  # Opponent has strong advantage
                    dangerous = True
        ...
        if debug_move:
            ...
            for eval_data in move_evaluations:
                try:
                    mv = Move.get((Move.game_id == gid) & (Move.move_hash == eval_data["move_hash"]))
                    meta = json.loads(mv.meta_json)
                    move_arr = deserialize_array(mv.move_bytes, meta["dtype"], json.dumps(meta["shape"]))
                    ...
                    debug_rows.append({
                        ...
                        "opponent_data_exists": bool(eval_data["opponent_data_exists"]),
                        "adjusted": bool(eval_data["adjusted"]),
                        "opponent_best_move": eval_data["opponent_best_move"],
                        "opponent_pW": float(eval_data["opponent_pW"]) if eval_data.get("opponent_pW") is not None else None,
                        "opponent_pT": float(eval_data["opponent_pT"]) if eval_data.get("opponent_pT") is not None else None,
                        "opponent_pN": float(eval_data["opponent_pN"]) if eval_data.get("opponent_pN") is not None else None,
                        "opponent_pL": float(eval_data["opponent_pL"]) if eval_data.get("opponent_pL") is not None else None,
                        "opponent_best_util_for_them": float(eval_data["opponent_util_for_them"]) 
                            if eval_data["opponent_util_for_them"] is not None else None,
                        "opponent_best_util_for_me": float(eval_data["opponent_util_for_me"]) 
                            if eval_data["opponent_util_for_me"] is not None else None,
                        "opponent_best_cert": float(eval_data["opponent_cert"]) 
                            if eval_data["opponent_cert"] is not None else None,
                        "dangerous": bool(eval_data["dangerous"]),
                        "is_best": eval_data["move_hash"] == best_move_hash,
                        "sort_key": (eval_data["adjusted_utility"], eval_data["total"], eval_data["certainty"]),
                    })
                except Exception:
                    continue
